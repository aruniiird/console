...
export const getPoolQuery = (poolNames: string[], queryName: string) => {
  const queries = {
    ...
    [StorageDashboardQuery.POOL_MAX_CAPACITY_AVAILABLE]: `ceph_pool_max_avail * on (pool_id) group_left(name)ceph_pool_metadata{name=~'${poolNames.join(
      '|',
    )}'}`,
    [StorageDashboardQuery.POOL_UTILIZATION_IOPS_QUERY]: `(rate(ceph_pool_wr[1m]) + rate(ceph_pool_rd[1m])) * on (pool_id) group_left(name)ceph_pool_metadata{name=~'${poolNames.join(
      '|',
    )}'}`,
    [StorageDashboardQuery.POOL_UTILIZATION_THROUGHPUT_QUERY]: `(rate(ceph_pool_wr_bytes[1m]) + rate(ceph_pool_rd_bytes[1m])) * on (pool_id) group_left(name)ceph_pool_metadata{name=~'${poolNames.join(
      '|',
    )}'}`,
    ...
    [StorageDashboardQuery.POOL_COMPRESSION_ELIGIBILITY]: `((sum(ceph_pool_compress_under_bytes > 0) / sum(ceph_pool_stored_raw and ceph_pool_compress_under_bytes > 0)) * 100) * on (pool_id) group_left(name)ceph_pool_metadata{name=~'${poolNames.join(
      '|',
    )}'}`,
